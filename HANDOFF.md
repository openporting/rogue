...
│  ├─ i18n.h / i18n.c          ← EN→KO `tr_msg()` + 조사 자동 처리 + 패턴 테이블.
│  │                              endmsg() 훅(패치 #2)에서 호출. `-DI18N_TEST`로 네이티브 테스트.
└─ web/
...
- ✅ **메시지 훅(패치 #2) + i18n.c 1차 적용**: `build.sh`가 `io.c`의 `endmsg()`를 자동 패치해
  `web_emit_msg(tr_msg(msgbuf))`로 보냄(상단 라인 그리기 대체). `webcurses/i18n.c`에 EN→KO
  패턴 테이블 + 조사 자동 처리(`han_has_batchim`/`$N{을}` 등) + 영문 폴백을 구현. 네이티브
  유닛 테스트(`gcc -DI18N_TEST i18n.c`) 통과. **단, 아직 1차**: §10.2 참고(전투 동사 변형 등 미번역분은 영문 폴백).
- ✅ **전투 메시지 한글화**: `fight.c`의 hit/miss/thunk/bounce 조립 문장(동사표 `h_names`/`m_names` +
  `prname`/`set_mname`) 전 형태를 `i18n.c` 패턴으로 커버. `@N` 액터 정규화로 관사("the ") 제거 +
  you/it/something 매핑 + **몬스터 이름 26종 한글화**(`MONSTERS[]`) → "박쥐를 맞혔다.",
  "트롤이 휘둘러 당신을 맞혔다." 등. 조사·어순까지 처리. 네이티브 테스트로 검증.
- ✅ **i18n 메시지 테이블 광범위 커버**: 전투/줍기/금화/함정(발견·발동)/장비(착용·해제·반지)/허기 단계/
  상태효과(획득·해제)/물약·두루마리·마법봉 효과/메두사 시선/계단/프롬프트/오류 등 `endmsg()`로 나가는
  플레이어 대상 메시지 대부분을 번역. 미번역은 영문 폴백. 커버리지 점검 하네스(`/tmp/cover` 패턴)로
  확인 — 남은 미번역은 대부분 조각(assembled 중간 토큰)·위저드/디버그·상태줄.
- ❌ **아직 안 된 것**:
  1. **아이템 이름 한글화**(물약/두루마리/무기/갑옷/반지 명칭). 메시지 본문은 번역됐지만 `%s`로 박히는
     아이템 이름은 아직 영어 → 조사도 무받침 기본. `MONSTERS[]`처럼 이름 사전 + 색/재질 처리 필요.
...
2. ✅(1차) **메시지 훅 + i18n.c**(패치 #2, §8) → `build.sh`가 `endmsg()`를 자동 패치해
   `web_emit_msg(tr_msg(msgbuf))` 호출. `webcurses/i18n.c`:
   - 엔진: 조립된 영어 문장을 `TRANSLATIONS[]`(영어 패턴→한국어 템플릿)에 풀-매칭. 패턴의
     `%s/%d/%c/...`를 캡처해 `$1..$9`로 재삽입, 매칭은 대소문자 무시(endmsg가 첫 글자 대문자화함).
   - 조사: `$N{을}`/`$N{이}`/`$N{은}`/`$N{과}`/`$N{으로}`/`$N{아}` — 캡처의 받침 유무로 자동 선택
     (`han_has_batchim`, `으로/로`는 ㄹ받침 처리). 한글이 아니면 무받침 기본.
   - 폴백: 매칭 없으면 원문 영어 그대로 반환(게임 진행 유지).
   - 테스트: `gcc -DI18N_TEST webcurses/i18n.c -o t && ./t`.
   - ✅ **전투 동사 처리 완료**: §8이 우려한 조각-concat 문제를 *소스 리팩터링 없이* i18n 레이어에서
     해결. `fight.c`를 패치하지 않고(외부 소스라 sed 함수 치환은 취약), 조립된 영어 전투 문장 전
     형태(excellent/injured/swing·terse/미사일 포함)를 패턴으로 매칭. 핵심은 `@N` **액터 정규화**:
     "the "/"a " 관사 제거 → you/it/something → `당신`/`그것`/`무언가` 매핑 → `MONSTERS[]`로 몬스터
     이름 26종 한글화. 그 뒤 `{이}`/`{을}` 조사를 한국어 결과의 받침으로 선택.
     · 순서 주의: 근접전 형태("you …"/"… you")를 일반 미사일 형태("the %s … %s")보다 **위에** 둬야
       동사구가 `%s`로 잘못 캡처되지 않음(코드 주석 참고).
     · 남은 미번역: 인벤/도움말/마법봉 일부 문구(영문 폴백). 몬스터 이름의 비전투 표기(상태줄·줍기 등)는
       이 `@N` 경로를 타지 않으므로 추후 `set_mname` 단계 한글화 또는 동일 사전 재사용으로 통일 가능.
...
- 메시지(패치 #2) 적용됨 — 단 i18n 테이블이 1차라 미번역 메시지는 영문으로 로그에 표시됨(폴백). 표 보강 필요(§10.2).
- `tr_msg` 패턴 테이블은 **순서 의존**: 더 구체적인 패턴(리터럴/인자 많은 것)을 위에 둬야 일반 패턴에 가려지지 않음. 예: `"you found %d gold pieces"`는 `"you found %s"`보다 먼저.

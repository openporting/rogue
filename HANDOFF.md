...
└─ web/
   ...
   ├─ ux-context-inventory-test.js ← 컨텍스트 인벤토리 E2E(pass/fail). 홈 화면 🎒소지품 →
   │                              아이템 탭 → 타입별 컨텍스트 메뉴(먹기/입기/벗기/장착/던지기/
   │                              버리기) → 명령+글자 자동 응답을 실 UI+브리지+엔진으로 검증.
   │                              던지기는 방향→아이템 핸드셰이크(엔진이 방향을 먼저 요구)까지 확인.
...
- ✅ **컨텍스트 인벤토리 (모바일 아이템 조작 UX 개편) — 완료**: 엔진(WASM) 무수정, 전부 `web/` 계층
  (`web/index.html` + 번역 1줄 `webcurses/i18n.c`). 기존엔 장착/던지기가 `더보기 → 장착 → *목록 →
  목록 확인 → 닫기 → 키패드에서 글자` 순이라 단계가 많고 가려진 프롬프트에 답해야 했음. 개편:
  - **홈 화면 `🎒 소지품` 버튼**: `더보기` 없이 한 번에 인벤토리 오픈(`I.inventory()`→`i`). 액션바의
    중복 `쉬기`는 제거(이미 D패드 중앙 `·`이 쉬기) → 2×3 그리드 유지.
  - **인터랙티브 인벤토리**: `getOverlay()`의 `a) …` 줄을 파싱해 **탭 가능한 행 + 타입 태그**(무기/
    방어구/물약/두루마리/반지/지팡이/화살/활/음식/부적)로 렌더(읽기전용 덤프 대체). 도움말·발견목록·
    죽음 화면은 기존 `<pre>` 그대로(아이템 줄 없으면 raw 모드).
  - **타입별 컨텍스트 메뉴**: 한글 아이템명으로 분류 → `actionsFor()`가 알맞은 동작 제시.
    음식→먹기, 물약→마시기, 두루마리→읽기, 지팡이→사용하기, 갑옷→입기/벗기(착용 중 표시 따라),
    반지→끼기/빼기, 무기→장착/던지기, 활→장착/던지기, 화살→던지기/장착, 모두 +버리기. `(착용 중)`/
    `(장착 중)` 마커로 입기↔벗기·장착↔던지기 전환.
  - **명령+글자 자동 응답**: 인벤토리는 키 대기로 블록돼 있으므로 액션 실행 시 `space`(인벤 닫기)+명령키를
    보내고, 그 명령이 띄우는 `get_item` 프롬프트를 **이미 고른 아이템 글자로 자동 응답**(`render()`의
    `pendingItemLetter` 주입). `T`(갑옷 벗기)/`R`(반지 빼기)는 글자 불필요 → 명령만 전송.
  - **던지기 방향→아이템 핸드셰이크**: 엔진은 `t` 시 **방향을 먼저** 묻고(`get_dir`) 그 다음 `missile()`이
    `get_item`을 띄움(`command.c`/`weapons.c` 확인). 그래서 던지기는 `space`+`t` 후 `#throw-hint` 배너로
    방향 입력을 안내하고(D패드/탭/스와이프가 방향키 전송), 방향 뒤에 뜨는 아이템 프롬프트에서 글자를
    자동 주입. 배너 탭=취소(ESC). `which direction? → "어느 방향으로? "` 번역 1줄 추가.
  - **검증**: `node web/ux-context-inventory-test.js`(실 UI+브리지+WASM 엔진) — 홈 버튼·인터랙티브 행·
    타입별 메뉴·장착(`w`)·갑옷 벗기/입기(`T`/`W`)·먹기(`e`)·던지기(`t`→방향→글자) **전 시나리오 PASS**.
    기존 회귀(`headless`/`more-prompt`/`e2e`/`persist`/`ux-walkthrough`/`ux-scenario-equip-inventory`)도 전부 PASS.
